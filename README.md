...
In the next version the certificate is automatically generated by the firmware and based on ECC which is more secure and more efficient. It contains the device's id, callsign and a .local domain name. It is self-signed by default, but it may also be able to use a REST API to have it signed by a CA. See [Polaric-CertAuth](https://github.com/PolaricServer/Polaric-Certauth).
